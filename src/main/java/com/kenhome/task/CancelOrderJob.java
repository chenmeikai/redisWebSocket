...
/**
 * @Author: cmk
 * @Description: 自动取消订单 思路（支持分布式） : 创建订单时，生成下订单id和自动取消到期时间的记录，将记录有序的放入redis的list链中，通过定时器从链的最先进的记录开始遍历，比较到期时间，如果到期则执行自动取消业务，执行完后继续遍历下个记录直至遍历到记录未达到到期时间，将该记录放回原来的list链的位置，不再执行遍历（因为最旧的记录没达到过期时间，其余的更无法达到过期时间，因为list是按时间有序的入栈的）；注意：当消费者手动取消订单和确认付款时，需要移除list对应的订单。
